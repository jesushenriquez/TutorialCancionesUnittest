...
class AlbumTestCase(unittest.TestCase):
   def setUp(self):
      ...
      self.album2 = Album(titulo='La voz de los 80s', ano=1984, descripcion='No tiene', medio=Medio.CASETE,
                          canciones=[])
      self.album3 = Album(titulo='Pateando piedras', ano=1986, descripcion='No tiene', medio=Medio.DISCO, canciones=[])
      self.album4 = Album(titulo='La cultura de la basura', ano=1987, descripcion='No tiene', medio=Medio.DISCO,
                          canciones=[])
   ...
   def test_agregar_album_repetido(self):
      ...
      resultado = self.coleccion.agregar_album(titulo="Corazones", anio=1985, descripcion="No tiene",
                                               medio=Medio.CASETE)
   ...
   def test_editar_album(self):
      ...
      resultado1 = self.coleccion.editar_album(album_id=1, titulo="Corazones Remastered", anio=1985,
                                               descripcion="No tiene", medio=Medio.CASETE)

      # Se cambia el título del segundo álbum creado por uno que ya existe
      resultado2 = self.coleccion.editar_album(album_id=2, titulo="Pateando piedras", anio=1985, descripcion="No tiene",
                                               medio=Medio.CASETE)
